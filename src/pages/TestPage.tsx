...
import { applySavitzkyGolayFilter, uniformlySpacedResample } from '../astro/tools/savitzkyGolayFilter';
...
const TestPage: React.FC = () => {
    ...
    const sPoints: DataSet = { points: sList, legendText: 'Raw datapoints', smoothenCurve: false, strokeScale: 1, showPoints: true, showLines: false, color: 'rgba(225, 202, 29, 0.3)' };

    const iList: { x: number, y: number }[] = Array(n).fill(0).map((_v: any, k) => {
        return { x: xi[k], y: yi[k] };
    });
    const iPoints: DataSet = { points: iList, legendText: 'Interpolated', smoothenCurve: false, strokeScale: 1, showPoints: true, color: 'rgba(74, 33, 133, 0.5)' };

    const sgList: { x: number, y: number }[] = Array(n).fill(0).map((_v: any, k) => {
        return { x: sg[0][k], y: sg[1][k] };
    });
    const sgPoints: DataSet = { points: sgList, legendText: 'Smoothed', smoothenCurve: false, strokeScale: 2, showPoints: false, color: 'red' };
